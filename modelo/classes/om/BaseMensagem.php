...
/**
 * Base class that represents a row from the 'mensagem' table.
 *
 * 
 *
 * @package    propel.generator..om
 */
abstract class BaseMensagem extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'MensagemPeer';
	...
	/**
	 * The value for the obrigatorio field.
	...
	/**
	 * Initializes internal state of BaseMensagem object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [obrigatorio] column value.
	 * 
	 * @return     string
	 */
	 ...
	 * @return     Mensagem The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = MensagemPeer::ID;
		}
	...
	public function setMensagem($v)
	{
		...
		if ($this->mensagem !== $v) {
			$this->mensagem = $v;
			$this->modifiedColumns[] = MensagemPeer::MENSAGEM;
		}
	...
	/**
	 * Set the value of [obrigatorio] column.
	 * 
	 * @param      string $v new value
	 * @return     Mensagem The current object (for fluent API support)
	 */
	public function setObrigatorio($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->obrigatorio !== $v) {
			$this->obrigatorio = $v;
			$this->modifiedColumns[] = MensagemPeer::OBRIGATORIO;
		}
	...
	public function setDataHora($v)
	{
		...
		if ($this->data_hora !== null || $dt !== null) {
			...
			if ($currentDateAsString !== $newDateAsString) {
				...
				$this->modifiedColumns[] = MensagemPeer::DATA_HORA;
			}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->mensagem = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->obrigatorio = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			...
			return $startcol + 4; // 4 = MensagemPeer::NUM_HYDRATE_COLUMNS.

		} catch (Exception $e) {
			throw new PropelException("Error populating Mensagem object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = MensagemPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = MensagemQuery::create()
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				MensagemPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(MensagemPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(MensagemPeer::MENSAGEM)) {
			$modifiedColumns[':p' . $index++]  = '`MENSAGEM`';
		}
		if ($this->isColumnModified(MensagemPeer::OBRIGATORIO)) {
			$modifiedColumns[':p' . $index++]  = '`OBRIGATORIO`';
		}
		if ($this->isColumnModified(MensagemPeer::DATA_HORA)) {
			$modifiedColumns[':p' . $index++]  = '`DATA_HORA`';
		}

		$sql = sprintf(
			'INSERT INTO `mensagem` (%s) VALUES (%s)',
			...
			foreach ($modifiedColumns as $identifier => $columnName) {
				switch ($columnName) {
					...
					case '`MENSAGEM`':
						...
						break;
					case '`OBRIGATORIO`':
						$stmt->bindValue($identifier, $this->obrigatorio, PDO::PARAM_STR);
						break;
					case '`DATA_HORA`':
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = MensagemPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = MensagemPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['Mensagem'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Mensagem'][$this->getPrimaryKey()] = true;
		$keys = MensagemPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getMensagem(),
			$keys[2] => $this->getObrigatorio(),
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setMensagem($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setObrigatorio($arr[$keys[2]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(MensagemPeer::ID)) $criteria->add(MensagemPeer::ID, $this->id);
		if ($this->isColumnModified(MensagemPeer::MENSAGEM)) $criteria->add(MensagemPeer::MENSAGEM, $this->mensagem);
		if ($this->isColumnModified(MensagemPeer::OBRIGATORIO)) $criteria->add(MensagemPeer::OBRIGATORIO, $this->obrigatorio);
		if ($this->isColumnModified(MensagemPeer::DATA_HORA)) $criteria->add(MensagemPeer::DATA_HORA, $this->data_hora);
	...
	{
		$criteria = new Criteria(MensagemPeer::DATABASE_NAME);
		$criteria->add(MensagemPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Mensagem (or compatible) type.
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setObrigatorio($this->getObrigatorio());
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Mensagem Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * If this Mensagem is new, it will return
	...
	public function setMensagemClientes(PropelCollection $mensagemClientes, PropelPDO $con = null)
	{
		...
		foreach ($mensagemClientes as $mensagemCliente) {
			...
			if ($mensagemCliente->isNew()) {
				$mensagemCliente->setMensagem($this);
			}
	 ...
	 * Otherwise if this Mensagem is new, it will return
	 * an empty collection; or if this Mensagem has previously
	 * been saved, it will retrieve related MensagemClientes from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in Mensagem.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 ...
	 * @return     PropelCollection|array MensagemCliente[] List of MensagemCliente objects
	 */
	public function getMensagemClientesJoinCliente($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = MensagemClienteQuery::create(null, $criteria);
	...
	{
		return (string) $this->exportTo(MensagemPeer::DEFAULT_STRING_FORMAT);
	}
